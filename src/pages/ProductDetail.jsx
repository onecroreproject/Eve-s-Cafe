  ...
  return (
    <>
      ...
      <Box sx={{ bgcolor: WHITE, minHeight: '100vh', fontFamily: F.sans, color: BLACK, pb: { xs: '80px', md: '96px' } }}>
        ...
        {/* ─── Accordion Sections ─── */}
        <Box sx={{ borderTop: `1px solid #f0f0f0`, mt: 4 }}>
          {/* Product Description */}
        ...
        <Box sx={{ py: 6, bgcolor: SAGE }}>
          <Container maxWidth="lg">
            <Box sx={{ textAlign: 'center', mb: 5 }}>
              <CustomChip>You May Also Love</CustomChip>
              <Typography sx={{ fontFamily: F.display, fontWeight: 700, fontSize: { xs: '1.8rem', md: '2.5rem' }, color: G, mt: 2 }}>
                Related <span style={{ fontStyle: 'italic', color: A }}>Treasures</span>
              </Typography>
            </Box>
            <Grid container spacing={3}>
              {relatedProducts.map((item) => (
                <Grid size={{ xs: 6, sm: 6, md: 3 }} key={item.slug}>
                  <Link to={`/product/${item.slug}`} style={{ textDecoration: 'none' }}>
                    <Box sx={{ bgcolor: WHITE, borderRadius: 2, overflow: 'hidden', border: `1px solid #f0f0f0`, transition: 'all 0.3s', '&:hover': { transform: 'translateY(-5px)', boxShadow: '0 10px 30px rgba(0,0,0,0.1)' } }}>
                      <Box sx={{ aspectRatio: '1/1', bgcolor: SAGE, p: 2 }}>
                        <img src={formatImg(item.image)} alt={item.name} style={{ width: '100%', height: '100%', objectFit: 'contain' }} />
                      </Box>
                      <Box sx={{ p: 2 }}>
                        <Typography sx={{ fontSize: '0.7rem', color: A, fontWeight: 700, mb: 1 }}>{item.category?.name}</Typography>
                        <Typography sx={{ fontSize: '0.85rem', fontWeight: 600, color: BLACK, mb: 1 }}>{item.name}</Typography>
                        <Typography sx={{ fontSize: '0.9rem', fontWeight: 700, color: G }}>₹{item.variants?.[0]?.price?.toLocaleString('en-IN')}</Typography>
                      </Box>
                    </Box>
                  </Link>
                </Grid>
